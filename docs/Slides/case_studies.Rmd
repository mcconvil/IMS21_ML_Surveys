...
**Goal**: Estimate the total number of bartenders in the US.
...
.mauve[What assisting model should we use?]
...
.mauve[Why should we not use linear regression?]
...
* The industry code is likely important and most bartenders probably only fall into a few of the industries.
...
* .mauve[Recursively] split sample into two groups based on an auxiliary variable.
...
At each end node, the estimator is given by the survey-weighted mean.
...
* Regression tree is .mauve[calibrated] to population totals of the end nodes:
...
* Regression tree estimator: End nodes of the tree are a set of .mauve[predictive] post-strata!
